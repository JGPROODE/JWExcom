...
class FileMoverApp:
    ...
    def create_listbox_with_buttons(self, root, row, column):
        ...
        listbox = tk.Listbox(frame, selectmode=tk.MULTIPLE, exportselection=0)
        listbox.pack(side=tk.LEFT)

        up_button = tk.Button(frame, text="Omhoog", command=lambda lb=listbox: self.move_item_up(lb))
        up_button.pack(side=tk.TOP)

        down_button = tk.Button(frame, text="Omlaag", command=lambda lb=listbox: self.move_item_down(lb))
