...
class FileMoverApp:
    def __init__(self, root, initial_csv_path=None, initial_source_folder=None):
        ...
        # Rode rand om de hele app
        self.root.configure(bg='#00FF00', bd=10, relief="ridge")
        ...
        tk.Label(root, text="Pad naar CSV-bestand:", bg='#00FF00').grid(row=0, column=0, padx=10, pady=10)
        tk.Entry(root, textvariable=self.csv_file_path, width=50).grid(row=0, column=1, padx=10, pady=10)
        tk.Button(root, text="Blader...", command=self.browse_csv, bg='#FF6347', fg='white').grid(row=0, column=2, padx=10, pady=10)  # Rode achtergrond

        tk.Label(root, text="Pad naar bronmap:", bg='#00FF00').grid(row=1, column=0, padx=10, pady=10)
        tk.Entry(root, textvariable=self.source_folder, width=50).grid(row=1, column=1, padx=10, pady=10)
        tk.Button(root, text="Blader...", command=self.browse_source, bg='#FF6347', fg='white').grid(row=1, column=2, padx=10, pady=10)  # Rode achtergrond

        tk.Button(root, text="Lees CSV-bestand", command=self.read_csv, bg='#FF6347', fg='white').grid(row=2, column=1, pady=10)  # Rode achtergrond

        tk.Label(root, text="Selecteer kolommen voor het pad:", bg='#00FF00').grid(row=3, column=0, padx=10, pady=10)
        self.path_listbox = self.create_listbox_with_buttons(root, 4, 0)

        tk.Label(root, text="Selecteer kolommen voor de studentmap:", bg='#00FF00').grid(row=3, column=1, padx=10, pady=10)
        self.name_listbox = self.create_listbox_with_buttons(root, 4, 1)

        tk.Label(root, text="Selecteer kolommen voor de prefix van de bestanden:", bg='#00FF00').grid(row=3, column=2, padx=10, pady=10)
        self.prefix_listbox = self.create_listbox_with_buttons(root, 4, 2)

        tk.Button(root, text="Verplaats bestanden", command=self.move_files, bg='#FF6347', fg='white').grid(row=5, column=1, pady=20)  # Rode achtergrond
    ...
    def create_listbox_with_buttons(self, root, row, column):
        frame = tk.Frame(root, bg='#00FF00', bd=5, relief="ridge")  # Lichtgroen kader, ridge relief voor een 3D-effect
        ...
        button_frame = tk.Frame(frame, bg='#00FF00')  # Lichtgroen kader
        button_frame.pack(side=tk.LEFT)

        up_button = tk.Button(button_frame, text="Omhoog", command=lambda lb=listbox: self.move_item_up(lb), bg='#' + '{:06x}'.format(random.randint(0, 0xFFFFFF)), fg='white')  # Willekeurige achtergrondkleur
        up_button.pack(side=tk.TOP, fill=tk.X)

        down_button = tk.Button(button_frame, text="Omlaag", command=lambda lb=listbox: self.move_item_down(lb), bg='#' + '{:06x}'.format(random.randint(0, 0xFFFFFF)), fg='white')  # Willekeurige achtergrondkleur
