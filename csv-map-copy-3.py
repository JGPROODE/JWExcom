...
class FolderGeneratorApp:
    def __init__(self, root):
        ...
        tk.Label(self.columns_frame, text="Selecteer kolommen:").grid(row=0, column=0, sticky="w")

        self.columns = {
            "Achternaam": tk.IntVar(),
            "Voorvoegsel": tk.IntVar(),
            "Voorletters": tk.IntVar(),
            "Studentnummer": tk.IntVar(),
            "Opleiding": tk.IntVar(),
            "Cohort": tk.IntVar(),
        }

        row_num = 1
        for col_name, var in self.columns.items():
            tk.Checkbutton(self.columns_frame, text=col_name, variable=var).grid(row=row_num, column=0, sticky="w")
            row_num += 1
    ...
    def generate_folders(self):
        ...
        try:
            ...
            selected_columns = [col_name for col_name, var in self.columns.items() if var.get()]
