...
class StudentFoldersGenerator:
    def __init__(self, root):
        self.root = root
        self.root.title("Student Folders Generator")

        # Frame voor studentgegevens
        self.student_frame = tk.Frame(root)
        self.student_frame.pack(pady=10)

        tk.Label(self.student_frame, text="Selecteer het CSV-bestand:").grid(row=0, column=0, sticky="w")
        self.csv_entry = tk.Entry(self.student_frame, width=50)
        self.csv_entry.grid(row=0, column=1, padx=10, pady=5)
        tk.Button(self.student_frame, text="Bladeren", command=self.browse_csv).grid(row=0, column=2)

        # Frame voor map met documenten
    ...
    def generate_folders(self):
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                tussenvoegsel = str(row['Voorvoegsel']) if not pd.isna(row['tussenvoegsel']) else ""
                voorletters = str(row['Voorletters'])
                opleiding_cohort = str(row['opleiding_cohort'])

                # Maak de mapnaam
                folder_name = f"{opleiding_cohort}/{achternaam}-{tussenvoegsel}-{voorletters}-{studentnummer}"
...
if __name__ == "__main__":
    root = tk.Tk()
    app = StudentFoldersGenerator(root)
    root.mainloop()
