...
class FileMoverApp:
    ...
    def create_listbox_with_buttons(self, root, row, column):
        frame = tk.Frame(root, bg='#C1FFC1')  # Lichtgroene achtergrondkleur
        ...
        up_button = tk.Button(frame, text="Omhoog", command=lambda lb=listbox: self.move_item_up(lb), bg='#8A2BE2', fg='white')  # Blauwe achtergrondkleur, witte tekst
        up_button.pack(side=tk.TOP)

        down_button = tk.Button(frame, text="Omlaag", command=lambda lb=listbox: self.move_item_down(lb), bg='#8A2BE2', fg='white')  # Blauwe achtergrondkleur, witte tekst
